...
const Dumpligator = () => {
  ...
  const generateIngredients = async () => {
    if (!dough) {
      generateSingleIngredient("podaj losowy opis ciasta na pierogi, nie pisz nic więcej chujku mały, i nie zaczynaj zdania od `oto losowe ciasto blablabla tylko podaj po prostu opis ciasta`", setDough);
    }
    if (!filling) {
      generateSingleIngredient("podaj jakieś losowe nadzienie do pierogów , nie pisz nic więcej i nie zaczynaj zdania od `oto losowe nadzienie do pierogów blablabla tylko podaj po prostu nadzienie`", setFilling);
    }
    if (!ingredients) {
      generateSingleIngredient("podaj składniki które są potrzebne do zrobienia tych pierogów z podanym wyżej ciastem i nadzieniem, nie pisz nic więcej i nie zaczynaj zdania od `oto składniki` tylko po prostu je podaj", setIngredients);
    }
  };
